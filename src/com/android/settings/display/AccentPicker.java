...
public class AccentPicker extends InstrumentedDialogFragment implements OnClickListener {
    ...
    private void initView() {
        ...
        if (mView != null) {
            ...
            blackAccent.setBackgroundColor(getResources().getColor(
                    (isUsingDarkTheme() || isUsingBlackTheme() || isUsingXtendedTheme()) ? R.color.accent_picker_white_accent : R.color.accent_picker_dark_accent));
    ...
    // Check for the black theme overlay
    private boolean isUsingXtendedTheme() {
        OverlayInfo themeInfo = null;
        try {
            themeInfo = mOverlayManager.getOverlayInfo("com.android.system.theme.xtended",
                    UserHandle.USER_CURRENT);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
